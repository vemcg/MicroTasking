...
5. **Tasking logic**
   ...
   - Done: the start of a new window occurrence un-pauses the queue if it was paused — that's the only thing it does to the pause state (it doesn't touch anything else about pausing). A pause only lasts for the window it was pressed in.
