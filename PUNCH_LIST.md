...
2. **Persistent application state**
   ...
   - Done: per-period (today/this week/this month/all-time) longest-streak stats now come from a real persisted completion log (`completion_log`: `CompletionRecord(epochMs, streakAtCompletion)` per completed task, in `TaskPool.kt`), trimmed to the last 45 days on every write. `longestStreakSince` picks the max streak reached within a given window; "all time" still just uses the existing `longest_streak` counter directly. `RAPID_TESTING_MODE` in `TaskScheduling.kt` is currently `true`, which shrinks "this week"/"this month" down to 7 minutes / minutes-matching-the-current-month's-day-count instead of 7 days / days-in-month, and drops the queue-delivery floor from 30 seconds to 5, for fast manual testing - flip it back to `false` before a real release build.
...
5. **Tasking logic**
   ...
   - Done: per-completion history for real per-period (today/this week/this month/all-time) stats — see item 2.
