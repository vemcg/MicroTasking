...
2. **Persistent application state**
   ...
   - Done: per-period (today/this week/this month/all-time) longest-streak stats now come from a real persisted completion log (`completion_log`: `CompletionRecord(epochMs, streakAtCompletion)` per completed task, in `TaskPool.kt`), trimmed to the last 45 days on every write. `longestStreakSince` picks the max streak reached within a given window; "all time" still just uses the existing `longest_streak` counter directly. `TESTING_FAST_SCORE_WINDOWS` in `TaskScheduling.kt` is currently `true`, which shrinks "this week"/"this month" down to 7/30 minutes instead of 7 days / days-in-month, for fast manual testing of the bucketing - flip it back to `false` before a real release build.
