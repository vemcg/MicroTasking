...
5. **Tasking logic**
   ...
   - Done: window close and window open are two independent checks now, not one — matters because otherwise the earliest opportunity to detect "a new window opened" also ends up being where "the old one closed" gets handled, which conflates two things that should stay separate:
     ...
     - Open (whenever a tick finds a new window occurrence has begun): resets the daily delivery count and un-pauses if paused. Nothing else — by design, if the queue somehow isn't empty at this point (it should always be, since close already cleared it), open does not abandon or score it; it's left alone and just built on top of.
