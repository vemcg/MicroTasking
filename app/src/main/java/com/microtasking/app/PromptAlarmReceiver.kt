...
class PromptAlarmReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        val preferences = TaskDelivery.prefs(context)
        if (preferences.getBoolean("app_in_foreground", false)) {
            // The foreground pacing loop owns delivery while the app is open; onStart/onStop
            // keep this alarm cancelled during that time, but reschedule defensively here too
            // in case one slipped through right at the foreground/background transition.
            Log.i("MicroTasking", "Skipping delivery while app is foregrounded")
